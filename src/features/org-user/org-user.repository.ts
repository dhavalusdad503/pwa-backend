...
export interface IOrgUserRepository {
  ...
  update(id: string, data: Partial<OrgUser>, transaction?: Transaction): Promise<OrgUser>;
  delete(id: string): Promise<boolean>;
  findAll(): Promise<OrgUser[]>;
  count(): Promise<number>;
}

class OrgUserRepository extends BaseRepository<OrgUser> implements IOrgUserRepository {
  constructor() {
    super(OrgUser);
  }
