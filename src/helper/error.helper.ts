import { UniqueConstraintError, ValidationError, ForeignKeyConstraintError, ExclusionConstraintError, DatabaseError } from 'sequelize';
...
export function extractErrorMessage(error: unknown, defaultMessage: string = 'An error occurred'): string {
    ...
    if (error instanceof ForeignKeyConstraintError) {
        ...
        return `Cannot ${parent?.routine === '_bt_check_unique' ? 'insert' : 'delete/update'} record: ${error.message}`;
    }
...
export function extractErrorStatus(error: unknown, defaultStatus: number = 400): number {
